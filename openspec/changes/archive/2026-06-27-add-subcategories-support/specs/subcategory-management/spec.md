...
The category creation UI SHALL display an optional parent category selector. The selector SHALL only show root categories matching the currently selected type. When a parent is selected, the type field SHALL become read-only.
